...
├── scripts/         # デプロイスクリプト
└── skills/          # ユーザーが明示的に起動するワークフロー（11種）
```
...
#### `/grill-me` — 設計の深掘りインタビュー

**使うとき**: 計画や設計をストレステストしたいとき。自分の考えに抜け漏れがないか、徹底的に質問攻めにしてほしいとき。

設計ツリーの各分岐を1つずつ辿りながら、依存関係を解決しつつ質問を繰り返す。各質問には推奨回答も提示される。コードベースから答えられる質問はコード探索で自動回答する。

```
/grill-me このマイクロサービス分割案
/grill-me 認証フローの設計
```
